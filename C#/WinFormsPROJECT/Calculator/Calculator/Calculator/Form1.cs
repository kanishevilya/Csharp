namespace Calculator {
    public partial class Form1 : Form {
		  ...
		  private System.Drawing.Point firstCoord;
		  private System.Drawing.Size TileSize;

		  private const int maxLength = 22;    //private char logicalSymbol;
                                             //private bool isAbleTo;
        private string firstString; private string secondString;
        private bool isChangedType;
        public Form1() {
            isChangedType = false;    //isAbleTo = true;
				firstCoord=new Point(0,0);
				TileSize=new Size(153, 109);
		  ...
		  private void isOneSimbole(char ch) {
		      if (!isCanToAddDigit()) return; if (this.Result.Text.Length == 1 && ch == '0' && this.Result.Text == "0") { return; } else if (this.Result.Text == "0" && ch != '0') { Result.Text = String.Empty; }
		      this.Result.Text += ch;
		  }
		  private void buttonsClick(char ch) {
		      this.LogicalMark.Text = ch.ToString(); 
				firstString = this.Result.Text;
				this.PreviousName.Text = firstString; 
				this.Result.Text = "0";
		  }
		  ...
		  private string calc() {
		      double tempNum = 0.0; double frst = double.Parse(firstString);
		      double sec = double.Parse(secondString); switch (this.LogicalMark.Text) {
		          case "+":
		          tempNum = frst + sec;
		          break;
		          case "-":
		  ...
		  private void buttonEqualClick(object sender, EventArgs e) {
		      if (this.LogicalMark.Text != String.Empty) {
		          ...
		          firstString = calc(); secondString = "0";
		          this.LogicalMark.Text = String.Empty; 
					 this.PreviousName.Text = "";
